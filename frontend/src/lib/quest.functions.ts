...
export const askMentorChat = createServerFn({ method: "POST" })
  .validator((data: {
    message: string;
    context?: { profile?: any; blueprint?: any };
  ...
  .handler(async ({ data }) => {
    ...
    for (const url of backendCandidates) {
      try {
        const res = await fetch(`${url}/api/gateway/chat`, {
          method: "POST",
    ...
    // Direct fallback if backend was temporarily unreachable
    try {
      const fallbackText = await generateText({
        system: `You are Yaduk, the senior AI Project Mentor and Architect.
You guide engineering students through architecting and building top-tier final-year projects.
Answer questions about implementation, tech stack, architecture, viva prep, and potential blockers.
Be encouraging, concrete, and clear.
CONTEXT:
${JSON.stringify(data.context || {}).slice(0, 8000)}`,
        prompt: data.message,
        agentName: "Yaduk Mentor Server Fallback Agent",
      });
